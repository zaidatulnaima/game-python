...
load_texture("assets/tiger win.png", wint_texture_id)
...
def set_light_position():
    light_position = [0.0, 0.0, 1.0, 0.0]  # Replace with your desired light position
...
def highlight_surrounding_squares(position):
    ...
    # Set material properties for the highlighted squares
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, (0.0, 0.0, 1.0, 0.5))
    glMaterialfv(GL_FRONT, GL_SPECULAR, (1.0, 1.0, 1.0, 1.0))
    glMaterialfv(GL_FRONT, GL_SHININESS, 50.0)

    glBindTexture(GL_TEXTURE_2D, kambing_texture_id)  # Use goat texture temporarily
    glBegin(GL_QUADS)
    ...
    for neighbor in get_neighbors(position):
        ...
        # Check if the neighbor is a goat adjacent to a tiger or adjacent to another goat
        is_adjacent_to_tiger = any(
            are_connected(neighbor, tiger_pos) and are_on_same_line(neighbor, tiger_pos, position)
            for tiger_pos in tiger_positions[0] + tiger_positions[1]
        )

        is_adjacent_to_goat = any(
            are_connected(neighbor, goat_pos) and are_on_same_line(neighbor, goat_pos, position)
            for goat_pos in goat_positions[0] + goat_positions[1]
        )

        if is_adjacent_to_tiger or is_adjacent_to_goat:
            glColor4f(1.0, 0.0, 0.0, 0.5)  # Highlight in red
        else:
            glColor4f(0.0, 0.0, 1.0, 0.5)  # Highlight in blue
...
def are_connected_diagonal(position1, position2):
    x1, y1 = position1
    x2, y2 = position2
    return (
        abs(x1 - x2) == 100 and abs(y1 - y2) == 100
        or abs(x1 - x2) == 100 and abs(y1 - y2) == 0
        or abs(x1 - x2) == 0 and abs(y1 - y2) == 100
    )
...
def main():
    ...
    while not game_over:
        ...
        # Check for game over condition (16 goats exited)
        if len(goat_positions[0]) + len(goat_positions[1]) == 20:
            game_over = True
            pygame.display.flip()
            wait_for_exit()  # Wait until the user clicks to exit

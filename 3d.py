...
kambing_model = Wavefront("assets/kambing3d")
singa_model = Wavefront("assets/singa3d")
...
# load_texture("assets/HARIMAU2.png", singa_texture_id)
# load_texture("assets/GOAT.png", kambing_texture_id)
...
def draw_tigers(positions):
    ...
    for pos in positions:
        ...
        glRotatef(90, 1, 0, 0)  # Sesuaikan dengan orientasi model obj
        draw_model(singa_model)
        glPopMatrix()
...
def draw_goats(positions):
    ...
    for pos in positions:
        ...
        glTranslatef(pos[0], pos[1], 0.0)
        glRotatef(90, 1, 0, 0)  # Sesuaikan dengan orientasi model obj
        draw_model(kambing_model)
        glPopMatrix()
        
def draw_model(model):
    for name, material in model.materials.items():
        glBegin(GL_TRIANGLES)
        for face in model.mesh_list[name].faces:
            for vertex_i in face:
                vertex = model.vertices[vertex_i]
                glNormal3f(*model.normals[vertex.normal])
                glTexCoord2f(*model.texcoords[vertex.texcoord])
